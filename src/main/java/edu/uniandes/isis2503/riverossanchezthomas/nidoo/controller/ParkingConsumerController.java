...
public class ParkingConsumerController  {
	...
	/**
	 * /parkingconsumers/create -> create a new parking consumer user and save it in
	 * the database.
	 * 
	 ...
	 * 
	 * @return A parkingconsumer describing if the user is succesfully updated or
	 *         not.
	 */
	@PutMapping("/update")
	@ResponseBody
	public ParkingConsumer updateParkingConsumer(@RequestBody @Valid ParkingConsumer parkingConsumer) {
		try {
			...
			String idParkingConsumer = parkingConsumer.getId();
			ParkingConsumer oldParkingConsumer = parkingConsumerDAO.findParkingConsumerById(idParkingConsumer);

			if (oldParkingConsumer == null)
				throw new Exception("The Parking Consumer with id:  " + idParkingConsumer + " doesn't exist");
	...
	public List<ParkingConsumer> getParkingConsumer() {
		try {
			List<ParkingConsumer> parkingConsumers = parkingConsumerDAO.findAll();
	...
	/**
	 * /parkingconsumers/delete/{id} -> delete the parkingconsumer user having
	 * thepassed id.
	 * 
	 * @param id The id for the person to delete
	 * @return A string describing if the parkingconsumer is succesfully deleted or
	 *         not.
	 */
	@DeleteMapping("/delete/{id}")
	@ResponseBody
	public String deleteParkingConsumer(@PathVariable("id") String id) {
		try {
	...
	@ResponseBody
	public String createParkingConsumerVehicle(@PathVariable("id") String id, @RequestBody @Valid Vehicle vehicle) {
		try {
			ParkingConsumer parkingConsumer = parkingConsumerDAO.findParkingConsumerById(id);
		...
		return "Vehicle with id: " + vehicle.getId() + " succesfully created! linked to Parking Consumer with id: "
				+ id;
	}

	@GetMapping("/getbyid/{idP}/vehicles/{idV}")
	@ResponseBody
	public Vehicle findParkingConsumerVehicle(@PathVariable("idP") String idP, @PathVariable("idV") String idV) {
		try {
			...
			if (vehicle == null)
				throw new Exception("The Vehicle with id:  " + idV + " doesn't exist");

			String idVehicleParkingConsumer = vehicle.getParkingConsumer().getId();
			String idParkingConsumer = parkingConsumer.getId();
			if (idVehicleParkingConsumer.equals(idParkingConsumer)) {
				throw new Exception("The Vehicle with id:  " + idV
						+ " doesn't have a connection with Parking Consumer with id: " + idP);
			}
			return vehicle;
		} catch (Exception e) {
			e.printStackTrace();
	...
	@ResponseBody
	public List<Vehicle> findParkingConsumerVehicles(@PathVariable("idP") String idP) {
		try {
			ParkingConsumer parkingConsumer = parkingConsumerDAO.findParkingConsumerById(idP);
	...
	@ResponseBody
	public Vehicle updateParkingConsumerVehicle(@PathVariable("idP") String idP, @RequestBody @Valid Vehicle vehicle) {
		try {
			ParkingConsumer parkingConsumer = parkingConsumerDAO.findParkingConsumerById(idP);
			...
			if (oldVehicle == null)
				throw new Exception("The Vehicle with id:  " + vehicle.getId() + " doesn't exist");
	...
	@ResponseBody
	public String deleteParkingConsumerVehicle(@PathVariable("idP") String idP, @PathVariable("idV") String idV) {
		try {
			ParkingConsumer parkingConsumer = parkingConsumerDAO.findParkingConsumerById(idP);
			...
			String idParkingConsumer = parkingConsumer.getId();
			if (idVehicleParkingConsumer.equals(idParkingConsumer)) {
				throw new Exception("The Vehicle with id:  " + vehicle.getId()
						+ " doesn't have a connection with Parking Consumer with id: " + idP);
			}
			...
			e.printStackTrace();
			return "Error deleting the Vehicle with id:" + idV + " with connection with Parking Consumer with id: "
					+ idP;
		}
		return "Vehicle with id: " + idV + " with connection with Parking Consumer with id: " + idP
				+ " succesfully deleted!";
	}
	...
	@ResponseBody
	public CorporationConsumer findCorporativeConsumersParkingConsumers(@PathVariable("id") String id) {
		try {
			ParkingConsumer parkingConsumer = parkingConsumerDAO.findParkingConsumerById(id);
			if (parkingConsumer == null) {
				throw new Exception("The parking Consumer with id:  " + id + "doesn´t exist");
