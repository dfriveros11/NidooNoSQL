...
public class CorporationConsumerController {
	...
	/**
	 * /corporationconsumer/create -> create a new corporationconsumer user and save
	 * it in the database.
	 * 
	 ...
	 * 
	 * @return the corporation consumer if it was succesfully updated and null if it
	 *         wasn´t updated.
	 */
	@PutMapping("/update")
	@ResponseBody
	public CorporationConsumer updateCorporationConsumer(@RequestBody @Valid CorporationConsumer corporationConsumer) {
		try {
			CorporationConsumer oldCorporationConsumer = corporationConsumerDAO
					.findCorporationConsumerById(corporationConsumer.getId());
			if (oldCorporationConsumer == null) {
				throw new Exception("The Corporative consumer with id: " + corporationConsumer.getId()
	...
	/**
	 * /corporationconsumer/allCorporationConsumers -> return all the
	...
	public List<CorporationConsumer> getCorporationConsumer() {
		try {
			List<CorporationConsumer> corporationConsumers = corporationConsumerDAO.findAll();
	...
	/**
	 * /corporationconsumer/delete/{id} -> delete the corporation consumer user
	 * having the passed id.
	 * 
	 * @param id The id for the user to delete
	 * @return A string describing if the corporation consumer is succesfully
	 *         deleted or not.
	 */
	@DeleteMapping("/delete/{id}")
	@ResponseBody
	public String deleteCorporationConsumer(@PathVariable("id") String id) {
		try {
	 ...
	 * 
	 * @param id              the id of the corporation consumer that you want to
	 *                        asociate with the new parking consumer
	 * @param parkingconsumer the parking consumer you want to create with all their
	 *                        attributes
	 * @exception returns a string that notifies that it doesn´t exists the
	 *                    corporation consumer with the id in the data base
	 * @return a string with the information if the new parking consumer is created
	 *         and related to the corporation consumer
	 */
	@PostMapping("/create/{id}/parkingconsumers")
	@ResponseBody
	public String createCorporationConsumerParkingConsumer(@PathVariable("id") String id,
			@RequestBody @Valid ParkingConsumer parkingconsumer) {
		try {
			CorporationConsumer corporativeConsumer = corporationConsumerDAO.findCorporationConsumerById(id);
			if (corporativeConsumer == null) {
		...
		return "Parking consumer with id " + parkingconsumer.getId()
				+ " succesfully created linked to Corporative Consumer with id: " + id;
	}

	/**
	 * Get the parking consumer with idP that is associated to the coporative
	 * consumer with id idC
	 * 
	 * @param idC id of the corporative consumer that is on search
	 * @param idP id of the parking consumer that is on search
	 * @exception that the corporative consumer you are trying to found doesn´t
	 *                 exist
	 * @return the parking consumer with idP and associated to the corporative
	 *         consumer with idC
	 */
	@GetMapping("/getbyid/{idC}/parkingconsumers/{idP}")
	@ResponseBody
	public ParkingConsumer findByIDCorporativeConsumerParkingConsumer(@PathVariable("idC") String idC,
	 ...
	 * @return the list of all parking consumers that are related to the parking
	 *         consumer
	 */
	@GetMapping("/all/{idC}/parkingconsumers")
	@ResponseBody
	public List<ParkingConsumer> findCorporativeConsumersParkingConsumers(@PathVariable("idC") String idC) {
		try {
			CorporationConsumer corporativeConsumer = corporationConsumerDAO.findCorporationConsumerById(idC);
			if (corporativeConsumer == null) {
	...
	/**
	 * Update the parking consumer with idP asociated to the corporative consumer
	 * with idC
	 * 
	 * @param idC             id of the corporative consumer
	 * @param parkingConsumer the parking consumer with all the new information for
	 *                        update
	 * @return the parking consumer with the updated information
	 */
	@PutMapping("/update/{idC}/parkingconsumers/{idP}")
	@ResponseBody
	public ParkingConsumer updateCorporativeConsumerParkingConsumer(@PathVariable("idC") String idC,
			@RequestBody @Valid ParkingConsumer parkingConsumer) {
		try {
			CorporationConsumer corporativeConsumer = corporationConsumerDAO.findCorporationConsumerById(idC);
			if (corporativeConsumer == null) {
	...
	/**
	 * Delete a parking consumer with idP associated to the corporative consumer
	 * with idC
	 * 
	 * @param idC id of the corporative consumer
	...
	@ResponseBody
	public String deleteCorporativeConsumerParkingConsumer(@PathVariable("idC") String idC,
			@PathVariable("idP") String idP) {
		try {
			CorporationConsumer corporativeConsumer = corporationConsumerDAO.findCorporationConsumerById(idC);
			if (corporativeConsumer == null) {
